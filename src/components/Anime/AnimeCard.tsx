...
export default function AnimeCard({ anime }: AnimeCardProps) {
  ...
  // ── Auto-translate synopsis saat modal terbuka ────────────────────────────
  // Menggunakan POST sesuai route /api/translate yang ada
  const translateSynopsis = async (text: string) => {
    if (!text || translatedSynopsis) return; // Sudah diterjemahkan, skip
    setIsTranslating(true);
    try {
      const res = await fetch('/api/translate', {
        method: 'POST',                              // ✅ POST bukan GET
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text }),              // ✅ body JSON bukan query param
      });
      if (!res.ok) throw new Error('Translate failed');
      const data = await res.json();
      setTranslatedSynopsis(data.translatedText || text); // ✅ key "translatedText"
    } catch {
      setTranslatedSynopsis(text); // Fallback ke teks asli
    } finally {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      // Auto-translate langsung saat modal terbuka, tidak tunggu klik tab
      if (attributes?.synopsis) {
        translateSynopsis(attributes.synopsis);
      }
          ...
          if (res.ok) {
            ...
            if (data?.data) {
              const combined = data.data.map((item: any) => {
                const charData = data.included?.find(
                  (inc: any) =>
                    inc.type === 'characters' &&
                    inc.id === item.relationships?.character?.data?.id
                );
                const personData = data.included?.find(
                  (inc: any) =>
                    inc.type === 'people' &&
                    inc.id === item.relationships?.person?.data?.id
                );
                return {
                  name: charData?.attributes?.name || 'Unknown',
                  image:
                    charData?.attributes?.image?.original ||
                    charData?.attributes?.image?.large ||
                    posterImage,
                  vaName: personData?.attributes?.name || 'Seiyuu TBD',
                  vaImage:
                    personData?.attributes?.image?.original ||
                    personData?.attributes?.image?.medium ||
                    null,
                };
              });
          ...
          if (res.ok) {
            ...
            if (data?.data) {
              const reviewsWithUsers = data.data.map((review: any) => ({
                ...review,
                user: data.included?.find(
                  (inc: any) =>
                    inc.type === 'users' &&
                    inc.id === review.relationships?.user?.data?.id
                ),
              }));
  ...
  const getPlatformInfo = (url: string) => {
    ...
    if (link.includes('crunchyroll'))
      return { label: 'Crunchyroll', logo: 'https://upload.wikimedia.org/wikipedia/commons/0/08/Crunchyroll_Logo.png', color: 'from-orange-600/20 to-orange-900/5 border-orange-500/20' };
    if (link.includes('netflix'))
      return { label: 'Netflix', logo: 'https://upload.wikimedia.org/wikipedia/commons/0/08/Netflix_2015_logo.svg', color: 'from-red-700/20 to-red-900/5 border-red-500/20' };
    if (link.includes('hulu'))
      return { label: 'Hulu', logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e4/Hulu_Logo.svg', color: 'from-green-700/20 to-green-900/5 border-green-500/20' };
    if (link.includes('disney'))
      return { label: 'Disney+', logo: 'https://upload.wikimedia.org/wikipedia/commons/3/3e/Disney%2B_logo.svg', color: 'from-blue-700/20 to-blue-900/5 border-blue-500/20' };
  ...
  return (
    <>
      ...
      <div
        ...
        className="group relative overflow-hidden rounded-2xl bg-[#0d1117] border border-white/5 hover:border-indigo-500/50 hover:shadow-[0_8px_40px_rgba(99,102,241,0.12)] cursor-pointer transition-all duration-500"
      >
        <div className="aspect-[3/4] overflow-hidden relative">
          ...
          {attributes?.averageRating && (
            <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-md border border-white/10 rounded-md px-1.5 py-0.5 text-white text-[8px] font-bold">
              ★ {attributes.averageRating}%
            </div>
          )}
        </div>
      ...
      {isPreviewOpen && (
        <div
          className="fixed inset-0 z-[99999] flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-md p-0 sm:p-4"
          onClick={() => setIsPreviewOpen(false)}
        >
          <div
            className="relative w-full sm:max-w-5xl xl:max-w-6xl h-[93dvh] sm:h-[88vh] bg-[#07090f] border border-white/[0.07] rounded-t-[28px] sm:rounded-3xl flex flex-col lg:flex-row overflow-hidden shadow-2xl animate-in slide-in-from-bottom-8 sm:zoom-in-95 duration-300"
            onClick={e => e.stopPropagation()}
          >
            {/* Tombol Close */}
            <button
              onClick={() => setIsPreviewOpen(false)}
              className="absolute top-4 right-4 z-50 w-8 h-8 flex items-center justify-center bg-white/[0.06] hover:bg-red-500 rounded-xl text-gray-400 hover:text-white transition-all"
            >
            ...
            <aside className="shrink-0 w-full lg:w-[350px] xl:w-[400px] flex flex-col overflow-y-auto no-scrollbar px-4 pt-8 pb-4 sm:px-5 lg:h-full border-b lg:border-b-0 lg:border-r border-white/[0.07] bg-gradient-to-b from-[#0b0e18] to-[#07090f]">
                ...
                {!isPlayingTrailer ? (
                  <>
                    ...
                    {youtubeId && (
                      ...
                      >
                        <div className="bg-red-600 p-4 rounded-full shadow-2xl group-hover:scale-110 transition-all shadow-red-950/60">
              ...
              <div className="hidden lg:block mt-8 pt-7 border-t border-white/[0.06] space-y-3.5">
                ...
                {detailData.map((d, i) => d.value && (
                  <div key={i} className="flex items-start gap-2.5 text-left">
                    <span className="text-[7px] text-gray-600 font-black uppercase tracking-widest w-[50px] shrink-0 mt-0.5">
                      {d.label}
                    </span>
                    <span className="text-[11px] text-gray-200 font-medium leading-snug break-words flex-1">
                      {d.value}
                    </span>
                  </div>
                ))}
              ...
              {/* ── Footer credit — hanya di sidebar ── */}
              <div className="hidden lg:flex mt-auto pt-6 items-center justify-between border-t border-white/[0.04]">
                <span className="text-[8px] text-gray-700 font-bold uppercase tracking-[0.25em]">
                  CoreAnime · Kodel Dev
                </span>
                <span className="text-[8px] text-gray-700 font-bold uppercase tracking-[0.25em]">
            ...
            <main className="flex-1 flex flex-col overflow-hidden bg-[#07090f]">
              <header className="shrink-0 px-4 sm:px-7 pt-3 sm:pt-6 border-b border-white/[0.07] bg-[#07090f]/95 backdrop-blur-md z-20">
                <h2 className="text-2xl sm:text-4xl font-black italic text-white uppercase tracking-tighter leading-[0.9] mb-6 text-left">
                  {attributes?.canonicalTitle || attributes?.title}
                </h2>
                <nav className="flex overflow-x-auto no-scrollbar gap-4">
                  {TABS.map(tab => (
                    <button
                      ...
                      className={`px-2 pb-3.5 text-[9px] font-black uppercase tracking-widest relative transition-all shrink-0 ${
                        activeTab === tab
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-7 no-scrollbar pb-10">
                {loadingDetail && activeTab !== 'Ringkasan' && activeTab !== 'Episode' ? (
                  <div className="py-20 text-center animate-pulse text-[10px] font-black uppercase tracking-widest opacity-20">
                    Memuat data...
                  </div>
                ) : (
                  <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 text-left">
                    ...
                    {activeTab === 'Ringkasan' && (
                      <div className="space-y-6">
                        {isTranslating ? (
                          ...
                          <p className="text-gray-300 text-[13px] sm:text-base leading-relaxed">
                      ...
                      <>
                        {loadingEpisodes && episodes.length === 0 ? (
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {episodes.map((ep: any, i: number) => (
                              <div key={i} className="flex gap-3 p-3 bg-white/[0.04] rounded-xl border border-white/[0.06]">
                                ...
                                <div className="flex flex-col justify-center min-w-0">
                                  <span className="text-indigo-400 text-[7px] font-black uppercase">
                                    EP {ep.attributes?.number}
                                  </span>
                                  <h4 className="text-white text-xs font-bold truncate italic">
                            ...
                            {loadingEpisodes && (
                              <div className="col-span-full py-4 text-center text-[8px] font-black uppercase tracking-widest opacity-30 animate-pulse">
                                Memuat sisa episode lainnya...
                              </div>
                            )}
                          </div>
                        ) : (
                          <p className="py-10 text-center text-gray-600 text-[10px] font-black uppercase tracking-widest italic">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-5 gap-4">
                          {characters.map((char: any, i: number) => (
                            <div key={i} className="flex flex-col gap-2">
                              <img
                                src={char.image}
                                className="aspect-[3/4] object-cover rounded-xl border border-white/5 shadow-lg"
                                alt=""
                              />
                              <p className="text-[10px] font-black text-white uppercase text-center truncate">
                                {char.name}
                              </p>
                              <div className="flex items-center gap-2 justify-center opacity-60">
                                <img
                                  ...
                                  alt=""
                                />
                                <span className="text-[8px] font-bold text-gray-400 uppercase truncate">
                                  {char.vaName}
                                </span>
                              </div>
                            </div>
                          ...
                          {reactions.map((review: any, i: number) => (
                            <div key={i} className="bg-white/4 p-6 sm:p-8 rounded-[2rem] border border-white/8 flex flex-col sm:flex-row gap-8">
                              <div className="w-full sm:w-32 shrink-0 flex flex-row sm:flex-col items-center gap-4 text-center">
                                <img
                                  src={review.user?.attributes?.avatar?.tiny || 'https://placehold.co/100x100?text=U'}
                          ...
                          {franchise.map((item: any, i: number) => (
                            <div key={i} className="bg-white/[0.04] hover:bg-indigo-600/10 hover:border-indigo-500/25 transition-all p-3 rounded-xl sm:rounded-2xl border border-white/[0.06] flex gap-3.5 items-center group/fr">
                              ...
                              <div className="min-w-0">
                                <p className="text-[7px] text-indigo-400 font-black uppercase mb-1 tracking-widest">
                                  {item.role}
                                </p>
                                <p className="text-[11px] text-white font-black line-clamp-2 uppercase italic leading-tight">
                    ...
                    {activeTab === 'Streaming' && (
                      streamingLinks.length > 0 ? (
                          ...
                          {streamingLinks.map((link: any, i: number) => {
                            ...
                            return (
                              <button
                                ...
                                className={`flex items-center justify-between bg-gradient-to-r ${platform.color} p-5 rounded-2xl border border-white/5 hover:brightness-125 transition-all`}
                              >
                                <div className="flex items-center gap-3">
                                  {platform.logo ? (
                                    <img src={platform.logo} className="h-4 object-contain" alt="" />
                                  ) : (
                        ...
                        <p className="py-10 text-center text-gray-600 text-[10px] font-black uppercase tracking-widest italic">
                          Link streaming tidak ditemukan.
                        </p>
                      )
                    )}
                  </div>
              ...
              {/* ── Footer credit — mobile (bawah konten utama) ── */}
